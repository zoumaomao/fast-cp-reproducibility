...
We also use augmented calibration data. In the main matched benchmark, each calibration series is augmented using the same corruption families as evaluation: missing segments, additive sensor noise, and amplitude drift. In the mismatched-corruption stress test, the calibration augmentation menu is intentionally restricted. The classifier is not retrained on these variants; they are used only to populate the calibration score and fingerprint pool.
