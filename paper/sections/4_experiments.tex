...
\Cref{tab:mismatch_corruption} shows that the efficiency trend persists under mismatch, but the operating boundary becomes clearer. On drift, where the global augmented threshold remains near nominal coverage, \fastcp{}-efficient reduces set size by 0.1252 labels while decreasing coverage by 0.57 percentage points. On mixed corruptions, both methods under-cover substantially, with augmented split conformal prediction at 0.8969 coverage and \fastcp{}-efficient at 0.8929. The correct interpretation is therefore not that \fastcp{} solves unseen corruptions; rather, mismatch should trigger validation and possible global fallback, especially when the global threshold itself is already below target.
